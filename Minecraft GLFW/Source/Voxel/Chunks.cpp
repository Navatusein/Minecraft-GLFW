...
Voxel* Chunks::get(int x, int y, int z) {
	...
	int ly = y - cy * ChunkH;
	int lz = z - cz * ChunkD;
	return &chunk->Voxels[(ly * ChunkD + lz) * ChunkW + lx];
}
...
void Chunks::set(int x, int y, int z, int id) {
	...
	int ly = y - cy * ChunkH;
	int lz = z - cz * ChunkD;
	chunk->Voxels[(ly * ChunkD + lz) * ChunkW + lx].id = id;
	chunk->Modified = true;
	...
	if (lx == ChunkW - 1 && (chunk = getChunk(cx + 1, cy, cz))) chunk->Modified = true;
	if (ly == ChunkH - 1 && (chunk = getChunk(cx, cy + 1, cz))) chunk->Modified = true;
	if (lz == ChunkD - 1 && (chunk = getChunk(cx, cy, cz + 1))) chunk->Modified = true;
}
